...
def detectar_marcador_en_imagen(imagen_path):
    """
    Detecta el marcador en la imagen usando OCR y detección visual
    Busca patrones como: 1, 2, A41, B11, C12, etc.
    """
    try:
        # Leer imagen
        ...
        # Buscar marcadores en esquinas (típicamente están ahí)
        h, w = gray.shape
        
        # Regiones a analizar (esquinas)
        regiones = [
            gray[0:int(h*0.2), int(w*0.8):w],  # Superior derecha
            gray[int(h*0.8):h, int(w*0.8):w],  # Inferior derecha
            gray[0:int(h*0.2), 0:int(w*0.2)],  # Superior izquierda
            gray[int(h*0.8):h, 0:int(w*0.2)],  # Inferior izquierda
        ]
        ...
        for region in regiones:
            ...
            try:
                ...
                # Buscar patrones de marcadores
                # Números simples: 1-9, 21-38
                patron_numero = re.findall(r'\b([1-9]|[1-3][0-9])\b', texto)
                
                # Letras con números: A41, B11, C12, etc.
                patron_letra = re.findall(r'\b([A-J]\d{1,2})\b', texto)
                
                # Número especial: 43, 45
        ...
        if match:
            ...
            if 535 <= numero_archivo <= 543:
                ...
                return str(numero_archivo - 534)  # 21-29
            elif numero_archivo == 565:
                return '43'
            elif 566 <= numero_archivo <= 574:
                return str(numero_archivo - 535)  # 31-38
...
def ordenar_marcadores(lista_marcadores):
    """
    Ordena los marcadores en el orden correcto:
    1-9, A41(10), B11-J19, 45(20), 21-29, 43(30), 31-38
    """
    def clave_orden(item):
        ...
        if len(marcador) == 3 and marcador[0].isalpha():
            letra = marcador[0]
            num = int(marcador[1:])
            orden_letra = ord(letra) - ord('B') + 11
...
def crear_mosaico_automatico(imagenes_ordenadas, folder, output_path):
    """
    Crea mosaico con las imágenes ordenadas automáticamente
    """
    add_log("Creando mosaico con imágenes ordenadas...", 'info')
    ...
    for idx, item in enumerate(imagenes_ordenadas):
        ...
        # Primera imagen: más porción
        if idx == 0:
            pct_inicio, pct_fin = 0.66, 1.0
        # Última imagen: menos porción
        elif idx == len(imagenes_ordenadas) - 1:
            pct_inicio, pct_fin = 0.5, 1.0
        # Intermedias: mitad inferior
        else:
            pct_inicio, pct_fin = 0.5, 1.0
        
        # Buscar archivo (máscara o original)
        ...
        if img is not None:
            ...
            add_log(f"✗ No se pudo cargar marcador {marcador}", 'warning')
...
def upload():
    try:
        ...
        for file in files:
            # ✅ CORRECCIÓN: Verificar que file.filename no sea None
...
def detect_markers():
    """Detecta marcadores en todas las imágenes subidas"""
    try:
        ...
        for filename in files:
            if allowed_file(filename):
                ...
                if marcador:
                    ...
                    add_log(f"  ✓ {filename} → Marcador: {marcador}", 'info')
                else:
                    add_log(f"  ✗ {filename} → No se detectó marcador", 'warning')
        ...
        return jsonify({
            'success': True,
            'markers': imagenes_ordenadas,
            'count': len(imagenes_ordenadas)
        })
...
def create_masks():
    """Convierte todas las imágenes a máscaras"""
    try:
        add_log("🎭 Creando máscaras...", 'info')
        
        imagenes = processing_state.get('detected_markers', [])
        if not imagenes:
            # Si no hay detección previa, usar todas las imágenes
            files = os.listdir(UPLOAD_FOLDER)
...
def create_mosaic():
    """Crea el mosaico con las imágenes ordenadas"""
    try:
        ...
        if not imagenes_ordenadas:
            return jsonify({
                'success': False,
                'error': 'Primero debes detectar los marcadores'
            }), 400
        
        # Crear mosaico de máscaras
...
if __name__ == '__main__':
    print("=" * 70)
    print("🤖 VERTICAL MOSAIC BUILDER - DETECCIÓN AUTOMÁTICA")
    print("=" * 70)
    print(f"📁 Base: {BASE_DIR}")
    print("=" * 70)
    print("✨ Características:")
    print("   • Detección automática de marcadores")
    print("   • OCR para identificar números y letras")
    print("   • Ordenamiento inteligente")
    print("   • Compatible con cualquier imagen DJI")
    print("=" * 70)
    
    # ✅ Configuración para Render (usa PORT de variable de entorno)
